...
use self::aux::get_optional_string_aux_tag;
use self::aux::sa_tag_parser::parse_sa_aux_val;
use self::cigar::{get_read_clip_positions, has_aligned_segments};
use crate::ChromList;
...
pub fn get_seq_order_read_split_segments(
    ...
    let (primary_read_size, mut seq_order_read_split_segments) = {
        let (read_start, read_end, read_size) = get_read_clip_positions(&record.cigar(), false);
    ...
    if let Some(sa_aux_val) = get_optional_string_aux_tag(record, b"SA") {
        ...
        for (sa_segment_index, sa_segment) in sa_segments.iter().enumerate() {
            ...
            let (read_start, read_end, read_size) =
                get_read_clip_positions(&sa_segment.cigar, false);
